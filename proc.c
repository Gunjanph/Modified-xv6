...
found:
	...
	*(uint*)sp = (uint)trapret;
	...
	p->rtime = 0;             // run time   
	p->iotime = 0;            // I/O time   
	p->numrun = 0;            // Number of times run   
	p->rabletime = 0;            // runnable time   
	p->current_queue = 0;
	p->last_run = ticks;
#ifdef MLFQ
	p->current_queue = 0;
...
exit(void)
{
	...
	curproc->etime = ticks;
#ifdef FCFS
	cprintf("#%d Start-time=%d;\tEnd-time=%d\n", curproc->pid, curproc->stime, curproc->etime);
#endif
...
int
set_priority(int jugad, int lul)
{
	int priority=jugad/1000;
	int pid=jugad%1000;
	struct proc *p;
	int ret=0;
	// cprintf("%d %d\n",priority, pid);
...
#ifdef MLFQ
struct proc* bestProcess(int *index1, int *index2, int *index3, int *index4, int *index5, uint *priority) {    
	struct proc* p;
	// cprintf("NPROC=%d\n", NPROC);
checkAgain:   
...
int quantum[5] = {1,2,4,8,16};

void incrementTimes(int ticks) {
	struct proc *p;   
	acquire(&ptable.lock);    
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
#ifdef MLFQ
		...
		switch(p->state) {    
			...
			case RUNNABLE:    
				...
				if(p->pid > 1)
				{
					if(ticks - p->last_run > DONTSTARVE)
					{
						if(p->priority-- == 0)
							p->priority = 0;
					}
			...
			case RUNNING:   
				...
				{ 
					p->tickcounter++;
					if(p->tickcounter == quantum[p->priority])
					{
						p->last_run = ticks;
						if(p->priority++ == 4)
							p->priority = 4;
				...
				// if(inctickcounter() == quantum[p->priority])
				//   decpriority();
#endif
				break;    
			default:  
